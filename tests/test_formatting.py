...
def test_print_player_enemy_info_pairs_related_stats_on_one_line(player, capsys):
    ...
    # Como porcentaje (150%), no como multiplicador (x1.50).
    assert "x1." not in out
    assert f"Daño Crítico: {player.get_total_crit_damage() * 100:.0f}%" in out
    assert f"Daño Crítico: {enemy.stats.crit_damage * 100:.0f}%" in out
