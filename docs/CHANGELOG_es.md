...
- **"Cazar..." + Explorar ponderado + frase de encuentro** (GDD §8.1, v0.14.0-d): una nueva opción "Cazar..." en el menú de zona lista los enemigos que ya has derrotado en la zona actual y te deja elegir uno directamente, sin la tirada de hallazgo de oro/poción de Explorar; el propio Explorar ahora pesa su encuentro aleatorio hacia los enemigos desbloqueados más avanzados de la zona en vez de un sorteo uniforme. Cada encuentro también trae una frase de sabor breve, estilo Pokémon, la primera vez que te topas con ese enemigo (más tenebrosa para élite/guardián, con una provocación desde el segundo encuentro si ya has perdido contra él antes) — se añadió una pausa después para que la frase no pase sin leerse. La pantalla de victoria ahora muestra totales en marcha (oro y progreso de nivel/XP, no solo el delta) y ya no revela el nombre del siguiente enemigo desbloqueado; Estadísticas y Equipar Armadura muestran ahora qué otorga cada pieza equipada, junto a su nombre.
- **Piedrablanca ya no puede acabar en combate** (fix, seguimiento de v0.14.0-d): el pueblo es un hub sin enemigos propios (GDD §3), pero Explorar ahí todavía podía acabar en una pelea contra cualquier enemigo desbloqueado — `Zone.is_hub` lo excluye ahora por completo de ese comportamiento de repuesto. Su hallazgo de oro/poción también queda limitado a uno de cada tipo por partida, en vez de repetirse para siempre sin enemigos que farmear.
- **Poder solo en modo DEBUG** (seguimiento de v0.14.0-d): activando la variable de entorno `VALETERNA_DEBUG` se muestra el poder de cada combatiente (`characters/power_budget.py`) bajo su ficha al empezar un combate — una ayuda de desarrollo, nunca visible de otro modo.
- **Bosque de los Susurros rellenado a 10 enemigos** (GDD §4.1/§4.7, v0.14.0-e): 7 enemigos nuevos — Araña Tejesombras, Druida Corrupto (élite), Oso Espectral, Enjambre de Polillas Pálidas (élite), Lobo Umbrío, Ent Corrompido (élite) y El Enraizado, el guardián de la zona, ligado a la lore del altar ya sembrada en las notas de la zona. Mecánica distintiva nueva: vida robada (el Oso Espectral se cura con una parte del daño que inflige en cada golpe, en vez de solo por debajo de un umbral de vida). Las estadísticas se dimensionaron para mantener la dificultad estrictamente progresiva zona a zona, en vez de seguir al pie de la letra la curva objetivo de la herramienta de presupuesto de poder, ya que esa curva reinicia baja en cada zona nueva mientras que el Troll, ya implementado, está muy por encima de su propio objetivo — documentado en `tests/test_power_budget.py` y en los archivos vecinos de `docs/design/presupuesto_de_poder.md`.
- **Poder del jugador ahora también en Estadísticas en modo DEBUG** (v0.14.0-e, ampliación): la misma línea `[DEBUG] Poder` que ya salía en la ficha previa al combate ahora también aparece en Personaje → Estadísticas, para poder comprobarlo fuera de combate.
- **Rata Gigante reforzada** (fix, v0.14.0-e, ampliación): el playtest mostró que se desbloquea justo después del Goblin pero se sentía más débil que él — su poder real ahora queda por encima del Goblin, en línea con el orden de desbloqueo.
- **Ciénaga de los Ahogados rellenada a 10 enemigos** (GDD §4.8, v0.14.0-f): esta zona no tenía roster alguno antes de ahora — 10 enemigos nuevos, Sanguijuela Colosal, Espantajo Anegado, Ahogado Errante, Chamán del Cieno (élite), Cangrejo Acorazado, Serpiente de Fango, Sacerdote Ahogado (élite), Horror de Profundidad, Guardián del Templo Hundido (élite) y El Anegado, el guardián de la zona (abre el Cañón del Trueno), ligados a los diálogos de Oren y a la nota de lore del templo hundido. El poder real de El Enraizado y el poder real original de Gárgola dejaban casi ningún margen para 10 tiers progresivos entre ambos, así que **se reforzó a Gárgola** (solo estadísticas) para abrir hueco suficiente, en vez de comprimir la dificultad de la zona nueva en una banda poco realista y casi plana — ver `docs/design/presupuesto_de_poder.md` y `_KNOWN_OUT_OF_RANGE` en `tests/test_power_budget.py` para el razonamiento completo.
- **Cañón del Trueno rellenado a 10 enemigos** (GDD §4.9, v0.15.0-a): 8 enemigos nuevos sobre la Gárgola y el Gólem de Piedra ya existentes — Minero Poseído, Murciélago de Tormenta, Chispa del Puntal (élite), Aparición de la Cuadrilla, Verdugo de la Mina (élite), Cabra Montés Corrupta, Heraldo de la Tormenta (élite) y El Decimoquinto, el guardián de la zona (abre la Torre de los Arcanos/Necrópolis), ligados a los diálogos y notas de lore de Kort sobre el derrumbe de la mina. Elemento distintivo nuevo para esta zona: rayo, incluyendo los primeros ataques de enemigo capaces de infligir `paralizado`. El siguiente enlace real de la cadena (Mago) ya tenía un poder real bajo documentado frente al de Gólem de Piedra (un punto ciego conocido de la fórmula, no algo que esta sub-fase intentara arreglar), así que los tiers nuevos suben progresivamente desde el poder real de Gólem de Piedra en su lugar.
- **Torre de los Arcanos/Necrópolis rellenada a 10 enemigos** (GDD §4.10, v0.15.0-b): 8 enemigos nuevos sobre el Mago y el Nigromante ya existentes — Tomo Viviente, Guardián Osario, Custodio Arcano (élite), Espectro de la Guardia, Bibliotecario Errante (élite), Carroñero de Cripta, Guardián del Tomo Prohibido (élite) y El Archivista, el guardián de la zona (abre la Ciudadela en Ruinas), ligados a los diálogos y notas de lore de Sella sobre el tomo prohibido desaparecido y los muertos que "se levantan por turnos" en la Cripta. Esta vez no hizo falta reforzar nada (a diferencia del Cañón) — pero el mismo tipo de transición rota y preexistente vuelve a aparecer un enlace más adelante: el poder real de Nigromante ya enlazaba directo con el de Ángel Caído (una inversión ya existente entre los 14 originales), así que los tiers nuevos suben progresivamente desde el poder real de Nigromante en su lugar, mismo razonamiento que el caso del Mago.
- **Ciudadela en Ruinas rellenada a 10 enemigos** (GDD §4.11, v0.15.0-c): 8 enemigos nuevos sobre el Ángel Caído y el Demonio ya existentes — Ciudadano Hueco, Guardia Caída, Serafín Corrupto (élite), Eco de la Guardia, Custodio de Vidrieras (élite), Verdugo Infernal, Heraldo del Amo (élite) y El Sin Rostro, el guardián de la zona (abre El Corazón de la Brecha), ligados a los diálogos y notas de lore de Aldric sobre los ciudadanos desaparecidos, su guardia perdida y el amo sin rostro al que sirven los ángeles caídos y temen los demonios. Esta vez el hueco no era un punto ciego de la fórmula — el poder real de Demonio y el del Dragón (jefe final) dejaban solo un ~29% de margen, demasiado ajustado para 8 tiers cómodos — así que, igual que con Gárgola en la Ciénaga, **se reforzó al Dragón** (solo estadísticas) para abrir hueco suficiente en vez de comprimir el roster nuevo en una subida antinaturalmente plana.
...
- **Herramienta de presupuesto de poder** (GDD §4.4, v0.14.0-b): una ayuda de diseño (no una pantalla del juego) que puntúa la amenaza intrínseca de un enemigo (`vida_máxima × velocidad × daño_neto`) y la compara con una curva objetivo ajustada al roster actual — se usará a partir de ahora para comprobar a ojo la tuneación de un enemigo nuevo antes de escribir su código, en vez de descubrirlo solo tras cientos de combates simulados. El Daño Crítico se muestra ahora como bonus (p. ej. `+60%`) en vez de multiplicador (`x1.60`) o total (`160%`) — en Bestiario, información de batalla y estadísticas.

- **Bestiario progresivo** (GDD §7.2, v0.14.0-a): la ficha de un enemigo se va completando según las veces que lo derrotas. **1 derrota**: línea de ambientación, vida, ataque, oro y los elementos de sus ataques. **3 derrotas**: el resto de estadísticas y su habilidad característica. **5 derrotas**: debilidades / resistencias / inmunidades, los estados que puede infligirte y a los que es inmune. **10 derrotas**: su tabla de botín completa con probabilidades. Un aviso indica cuántas victorias más revelan el siguiente escalón. Los 14 enemigos tienen ya su línea de ambientación y la descripción de su habilidad en una frase.
