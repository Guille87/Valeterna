...
- [ ] **Pendiente (v0.11+): música por élite/guardián, no por dificultad.** Cuando existan los tiers de zona (§ GDD 7.4: estándar / élite 5-7-9 / guardián 10), la música de combate la activarán los élites y guardianes, no la dificultad calibrada a mano. Los enemigos estándar de zona nunca cortarán la pista. `has_dedicated_battle_music()` es el punto único a cambiar; hoy devuelve `enemy in HARD_BATTLE_ENEMIES or enemy == FINAL_BOSS` como aproximación. Nota v0.16.0: los tiers de zona (élite = tier 5/7/9) ya existen (`ENCOUNTER_KIND`), pero `HARD_BATTLE_ENEMIES` (música) se dejó deliberadamente sin tocar en esa pasada — sigue pendiente si se quiere dar música dedicada a los 18 élites nuevos.
- [x] **4 retoques pequeños de UX/economía pedidos por el usuario tras una sesión de juego** (rama `polish/small-ux-fixes`):
  - **Aviso de habilidad nueva al subir de nivel**: `Player._level_up()` ahora anuncia (nombre + descripción, mismo formato que el menú Habilidades) cualquier habilidad cuyo nivel de desbloqueo sea justo el nivel alcanzado — antes no se avisaba de nada, solo se descubría entrando al menú por tu cuenta.
  - **Inventario**: los materiales llevan ahora `(material)` a la derecha de su descripción, para distinguirlos de un consumible de un vistazo.
  - **Tienda → Vender**: la lista ahora marca con `(E)` el objeto equipado, igual que ya hacía el inventario general.
  - **Espada de Hierro rebalanceada**: dañaba lo mismo (+2) que la Espada Goblin pese a ser mucho más fácil de conseguir (comprarla con oro vs. un drop con RNG); bajada a +1 de daño y a un valor de reventa más bajo (4, antes 6).
