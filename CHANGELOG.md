...
- **"Cazar..." + weighted Explorar + encounter flavor text** (GDD §8.1, v0.14.0-d): a new "Cazar..." zone-menu option lists enemies you've already defeated in the current zone and lets you pick one directly, without Explorar's gold/potion discovery roll; Explorar itself now weights its random encounter toward the zone's more-advanced unlocked enemies instead of a uniform roll. Every encounter also gets a short, Pokémon-style flavor line the first time you meet that enemy (more ominous for elite/guardián, with a taunt from the second encounter on if you've lost to it before) — a pause was added afterward so the line doesn't flash by unread. The victory screen now shows running totals (gold and level/XP progress, not just the delta) and no longer spoils the name of the next enemy unlocked; Estadísticas and Equipar Armadura now show what each equipped piece grants, next to its name.
- **Piedrablanca no longer triggers combat** (fix, v0.14.0-d follow-up): the village is a hub with no enemies of its own (GDD §3), but Explorar there could still roll a fight against any unlocked enemy — `Zone.is_hub` now excludes it from that fallback entirely. Its gold/potion discovery is also capped to one of each per save, instead of being repeatable forever with no enemies to farm.
- **DEBUG-only power display** (v0.14.0-d follow-up): setting the `VALETERNA_DEBUG` environment variable shows each combatant's power score (`characters/power_budget.py`) under their stat sheet at the start of a battle — a development aid, never shown otherwise.
- **Bosque de los Susurros fleshed out to 10 enemies** (GDD §4.1/§4.7, v0.14.0-e): 7 new enemies — Araña Tejesombras, Druida Corrupto (elite), Oso Espectral, Enjambre de Polillas Pálidas (elite), Lobo Umbrío, Ent Corrompido (elite) and El Enraizado, the zone's guardian, tied to the altar lore already seeded in the zone's notes. New signature mechanic: life drain (Oso Espectral heals for a share of the damage it deals on every hit, instead of only below a health threshold). Stats were sized to keep difficulty strictly progressive zone-over-zone rather than matching the power-budget tool's target curve literally, since the curve resets low at each new zone while the already-implemented Troll sits well above its own target — documented in `tests/test_power_budget.py` and `docs/design/presupuesto_de_poder.md`'s neighbouring files.
- **Player power now also shown in Estadísticas under DEBUG** (v0.14.0-e follow-up): the same `[DEBUG] Poder` line already shown on the pre-battle sheet now also appears in Personaje → Estadísticas, so it can be checked outside combat.
- **Rata Gigante reinforced** (fix, v0.14.0-e follow-up): playtesting showed it unlocks right after the Goblin but felt weaker than it — its real power now sits above the Goblin's, matching unlock order.
- **Ciénaga de los Ahogados fleshed out to 10 enemies** (GDD §4.8, v0.14.0-f): this zone had no roster at all before now — 10 new enemies, Sanguijuela Colosal, Espantajo Anegado, Ahogado Errante, Chamán del Cieno (elite), Cangrejo Acorazado, Serpiente de Fango, Sacerdote Ahogado (elite), Horror de Profundidad, Guardián del Templo Hundido (elite) and El Anegado, the zone's guardian (opens Cañón del Trueno), tied to Oren's dialogue and the drowned-temple lore note. El Enraizado's real power and Gárgola's original real power left almost no room for 10 progressive tiers between them, so **Gárgola was reinforced** (stats only) to open enough headroom, rather than compressing the new zone's difficulty into an unrealistically flat band — see `docs/design/presupuesto_de_poder.md` and `tests/test_power_budget.py`'s `_KNOWN_OUT_OF_RANGE` for the full reasoning.
- **Cañón del Trueno fleshed out to 10 enemies** (GDD §4.9, v0.15.0-a): 8 new enemies on top of the pre-existing Gárgola and Gólem de Piedra — Minero Poseído, Murciélago de Tormenta, Chispa del Puntal (elite), Aparición de la Cuadrilla, Verdugo de la Mina (elite), Cabra Montés Corrupta, Heraldo de la Tormenta (elite) and El Decimoquinto, the zone's guardian (opens Torre de los Arcanos/Necrópolis), tied to Kort's dialogue and lore notes about the mine collapse. New signature element for this zone: `rayo` (lightning), including the first enemy attacks that can inflict `paralizado`. The following real link in the chain (Mago) has an already-documented low real power relative to Gólem de Piedra (a known formula blind spot, not something this sub-phase tried to fix), so the new tiers climb progressively from Gólem de Piedra's real power instead.
- **Torre de los Arcanos/Necrópolis fleshed out to 10 enemies** (GDD §4.10, v0.15.0-b): 8 new enemies on top of the pre-existing Mago and Nigromante — Tomo Viviente, Guardián Osario, Custodio Arcano (elite), Espectro de la Guardia, Bibliotecario Errante (elite), Carroñero de Cripta, Guardián del Tomo Prohibido (elite) and El Archivista, the zone's guardian (opens Ciudadela en Ruinas), tied to Sella's dialogue and lore notes about the missing forbidden tome and the dead who "rise in shifts" in the Cripta. No reinforcement was needed this time (unlike the Cañón) — but the same kind of pre-existing broken transition recurs one link further on: Nigromante's real power already linked directly to Ángel Caído's (an existing inversion among the original 14), so the new tiers climb progressively from Nigromante's real power instead, same reasoning as the Mago case.
- **Ciudadela en Ruinas fleshed out to 10 enemies** (GDD §4.11, v0.15.0-c): 8 new enemies on top of the pre-existing Ángel Caído and Demonio — Ciudadano Hueco, Guardia Caída, Serafín Corrupto (elite), Eco de la Guardia, Custodio de Vidrieras (elite), Verdugo Infernal, Heraldo del Amo (elite) and El Sin Rostro, the zone's guardian (opens El Corazón de la Brecha), tied to Aldric's dialogue and lore notes about the vanished citizens, his lost guard, and the unseen ruler that fallen angels serve and demons fear. This time the gap wasn't a formula blind spot — Demonio's real power and the Dragón's (final boss) left only ~29% of headroom, too tight for 8 comfortable tiers — so, same as with Gárgola in the Ciénaga, **the Dragón was reinforced** (stats only) to open enough room instead of compressing the new roster into an unnaturally flat climb.
